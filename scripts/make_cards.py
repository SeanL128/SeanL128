...
(neofetch-style panel). Pure stdlib. Animation is CSS keyframes inside each
SVG; every element is visible by default and keyframes only add the reveal
(fill-mode: backwards), so renderers that pause animations show the final
frame instead of a blank card. Amber-phosphor CRT palette."""
...
def chrome(w, h, title):
    return (
        ...
        f'<circle cx="22" cy="17" r="5.5" fill="#4a3a1e"/><circle cx="42" cy="17" r="5.5" fill="#4a3a1e"/>'
        f'<circle cx="62" cy="17" r="5.5" fill="{AMBER_DIM}"/>'
...
def typed_line(x, y, prompt, text, start, fill=INK, size=15):
    ...
    parts = []
    if prompt:
        parts.append(f'<text x="{x}" y="{y}" font-size="{size}" fill="{AMBER}">{esc(prompt)}</text>')
    px = x + 9.1 * len(prompt)
    tspans = "".join(
...
def header_svg():
    ...
    lines.append(l1)
    l2, t = typed_line(28, 106, "", "Sean Lindsay", t + 0.35, fill=AMBER, size=26)
    lines.append(l2)
    ...
    lines.append(l4)
    cursor = (
        f'<rect x="{28 + 8.0 * 54:.0f}" y="154" width="8" height="15" fill="{AMBER}" '
...
def info_card_svg():
    w, h = 410, 266
    rows = [
        ("os", "macOS / wherever agents run"),
        ("shell", "claude (fable-5)"),
        ("focus", "AI agent tooling, plugins"),
    ...
    return (
        ...
        f'{STYLE}{chrome(w, h, "neofetch")}' + "".join(body) + "</svg>"
    )
